...
export default function BuyerPanel() {
  ...
  const spiceInfo = {
    cardamom: { label: "Astroids", image: "/images/cardamom.png" },
    pepper: { label: "Lunar Vehicles", image: "/images/pepper.png" },
    clove: { label: "Lunar Particles", image: "/images/clove.png" },
    nutmeg: { label: "Space Suits", image: "/images/nutmeg.png" },
    cinnamon: { label: "Rocket-Scrap", image: "/images/cinnamon.png" },
    vanilla: { label: "Astronaut autographs", image: "/images/vanilla.png" }
  };
  ...
  const handleOrder = async (listing) => {
    try {
      const orderData = {
        ...
        price: listing.price,
        farmerId: listing.userId || 'unknown',
      ...
      alert(`❌ Order failed. Calling seller at direct contact. ${listing.phone}`);
                ...
                return (
                  <div key={listing.id} className="card p-6 hover:shadow-strong transition-all duration-300">
                    <div className="flex items-start gap-4">
                      <img
                        src={spice?.image || '/images/placeholder.svg'}
                        alt={spice?.label}
